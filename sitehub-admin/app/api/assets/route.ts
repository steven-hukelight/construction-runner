...
async function ensureCreateAccess(req?: Request): Promise<{ companyId: string | null; error: NextResponse | null }> {
  const cookieStore = await cookies();
...
export async function POST(req: Request) {
  try {
    const { companyId, error } = await ensureCreateAccess(req);
    if (error) return error;
    if (!companyId) {
    ...
    const siteId = body?.site_id ?? body?.siteId ?? null;

-- Fix type mismatches in perform_attendance_fallback() (timestamp coalesce, site join).
--
-- NOTE: Do not use `20260406_fix_fallback_types.sql` as the filename — lexicographic order would
-- run it *before* 20260407120000 / 20260408120000 and overwrite a newer function definition.
--
-- vs draft: we do NOT use `s.id = rec.site_id::uuid`. attendance.site_id is TEXT (opaque ids);
-- casting to uuid errors for non-UUID values. Use `s.id::text = trim(rec.site_id::text)` instead.

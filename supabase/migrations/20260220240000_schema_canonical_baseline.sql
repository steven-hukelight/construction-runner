...
-- =============================================================================
-- 3. MISSING COLUMNS (drift report exact types)
-- =============================================================================
...
ALTER TABLE notices ADD COLUMN IF NOT EXISTS site_id TEXT;
...
-- =============================================================================
-- 4. TYPE CORRECTIONS (issue_date, expiry_date -> DATE per live)
-- =============================================================================

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'profile_training' AND column_name = 'issue_date' AND data_type != 'date') THEN
    ALTER TABLE profile_training ALTER COLUMN issue_date TYPE DATE USING issue_date::date;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'profile_training' AND column_name = 'expiry_date' AND data_type != 'date') THEN
    ALTER TABLE profile_training ALTER COLUMN expiry_date TYPE DATE USING expiry_date::date;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'profile_certifications' AND column_name = 'issue_date' AND data_type != 'date') THEN
    ALTER TABLE profile_certifications ALTER COLUMN issue_date TYPE DATE USING issue_date::date;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'profile_certifications' AND column_name = 'expiry_date' AND data_type != 'date') THEN
...
-- =============================================================================
-- 6. RLS POLICIES (16 tables with RLS enabled but no policies)
-- =============================================================================
...
-- audit_logs (user_id may be TEXT or UUID - use text comparison)

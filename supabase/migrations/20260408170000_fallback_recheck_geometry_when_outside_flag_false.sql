-- perform_attendance_fallback: when last_location_outside_fence = false, the previous code
-- treated the user as permanently "inside" and never re-evaluated coordinates. Foreground
-- pings while on site set the flag to false; if the operative leaves without a ping that
-- sets true (app killed, geofence delay, etc.), stale_ok could be true for minutes with no
-- auto sign-out. After stale_ok we already trust that location data is old — re-check
-- point_inside_site using last known lat/lng (same as NULL flag / legacy path).

-- 1. Add has_medical_issues: if false/No, medical complete without certificate.
--    If true/Yes, medical certificate required. Backward compat: null + fit_to_work=true = no issues.
ALTER TABLE pre_induction_medical ADD COLUMN IF NOT EXISTS has_medical_issues BOOLEAN;

-- 2. Update status/score: RTW +30 when (passport OR visa) + proof (not only when verified).
--    Medical +20 when complete: no issues (has_medical_issues=false OR fit_to_work=true) OR cert OR verified.
...
declare
  ...
  rtw_has_id boolean := false;
  rtw_has_proof boolean := false;
  rtw_verified boolean := false;
  med_no_issues boolean := false;
  med_has_cert boolean := false;
  med_verified boolean := false;
begin
  ...
  -- Right to Work: passport OR visa (either suffices) + proof_of_address. Visa only needed if no passport.
  rtw_has_id := rtw is not null and (
    (rtw.passport_url is not null and trim(rtw.passport_url) <> '')
    or (rtw.visa_url is not null and trim(rtw.visa_url) <> '')
  );
  rtw_has_proof := rtw is not null and (rtw.proof_of_address_url is not null and trim(rtw.proof_of_address_url) <> '');
  rtw_verified := rtw is not null and coalesce(rtw.right_to_work_verified, false) = true;
  rtw_complete := rtw is not null and (rtw_verified or (rtw_has_id and rtw_has_proof));
  ...
  -- Medical: No issues (has_medical_issues=false OR fit_to_work=true) = complete without cert.
  -- If has issues, need certificate or admin verified. Backward compat: fit_to_work=true = no issues.
  med_no_issues := medical is not null and (
    coalesce(medical.has_medical_issues, true) = false
    or medical.fit_to_work is true
    or coalesce(medical.fit_to_work::text, '') = 'true'
  );
  med_has_cert := medical is not null and (medical.medical_certificate_url is not null and trim(medical.medical_certificate_url) <> '');
  med_verified := medical is not null and coalesce(medical.medical_verified, false) = true;
  medical_complete := medical is not null and (med_verified or med_no_issues or med_has_cert);
  ...
  -- Score: award points when section is complete (not only when admin-verified)
  if rtw_complete then score := score + 30; end if;
  if medical_complete then score := score + 20; end if;
